...
export default function ProductShowcase() {
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {filtered.map((product, i) => (
              ...
              >
                ...
                >
                  <div className="bg-white border border-border rounded-lg overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1 cursor-pointer">
                    {/* Image — adapts to the natural size of your photo */}
                    <div className="w-full bg-surface relative overflow-hidden">
                      {/* Fallback placeholder shown only when image is missing */}
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-text-tertiary py-12">
                      ...
                      <img
                        src={product.image}
                        alt={product.name}
                        className="relative z-10 w-full h-auto block group-hover:scale-105 transition-transform duration-500"
                        onError={(e) => {
      ...
      {/* Slide-over panel — renders outside the section via AnimatePresence */}
      <ProductPanel
